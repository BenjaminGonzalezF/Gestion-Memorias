...
import Funcionario from '../models/Profesores';

// Agregar una nota
router.post('/nuevo_profesor', async(req, res) => {
  const body = req.body;  
  try {
...
router.get('/todos_profesores', async(req, res) => {
  try {
...
router.delete('/profesor_el/:id', async(req, res) => {
  const _id = req.params.id;
  try {
...
router.put('/profesor_ac/:id', async(req, res) => {
  const _id = req.params.id;
  const body = req.body;
  try {
